...
> **Last verified:** 2026-07-24. Support status is determined primarily from the **live EKS API**; the dated table below is a fallback used only when the API is unavailable.
...
**Version out-of-range guard (fallback mode only):** If using the fallback table and the cluster version is higher than the highest version in the table, rate GREEN and note: "Version 1.X is newer than the fallback table (last verified 2026-07-24); the live API was unavailable. Rated GREEN as latest. Refresh the table when convenient."
