...
Run `aws eks describe-cluster-versions` to get the authoritative real-time list of EKS versions and each version's `status` (`STANDARD_SUPPORT` / `EXTENDED_SUPPORT` / `UNSUPPORTED`). Define **latest** = the highest version whose status is `STANDARD_SUPPORT` in the API response.
...
2. Run `aws eks describe-cluster-versions` → find the cluster's version and its `status`, and identify **latest** = the highest `STANDARD_SUPPORT` version. If the live API is unavailable, fall back to the dated table above and note fallback mode in the finding.
