...
def _autolink_sub(match):
    """Render a bare <https://...> autolink as an anchor.

    The captured URL is already HTML-escaped (the caller escapes before
    inline formatting), so it is safe as both the visible text and, after
    scheme-sanitizing, the href.
    """
    raw = match.group(1)
    url = _safe_href(raw)
    return f'<a href="{url}">{raw}</a>'
...
def inline_format(text):
    ...
    def _link_sub_local(match):
        ...
        return f'<a href="{_safe_href(url_resolved)}">{text_g}</a>'
    ...
    # literal text.
    text = re.sub(r'&lt;(https?://[^\s]+?)&gt;', _autolink_sub, text)
    # Bold+italic: ***text*** — must run before the ** rule, otherwise the
    # leading ** is consumed and a stray * is left behind.
    text = re.sub(r'\*\*\*(.+?)\*\*\*', r'<strong><em>\1</em></strong>', text)
    # Bold: **text**
    text = re.sub(r'\*\*(.+?)\*\*', r'<strong>\1</strong>', text)
    # Italic: *text* — the opening * must be at start-of-string or preceded by
    # whitespace, and the closing * must be at end-of-string or followed by
    # whitespace/sentence punctuation; both delimiters hug non-space content.
    # This matches genuine *emphasis* while leaving IAM wildcards untouched:
    # in arn:aws:eks:*:...:cluster/*, eks:Describe*, and 5*3 the stars are
    # glued to identifiers (:, /, digits) so no * can ever open emphasis.
    # (after bold, so ** pairs are already converted and cannot be eaten)
    text = re.sub(r'(?<![^\s([])\*(\S(?:[^*]*\S)?)\*(?=[\s.,;:!?)\]]|$)', r'<em>\1</em>', text)
    # RAG emoji badges
...
def _split_row_cells(line, ncols=None):
    ...
    for cell in aware:
        if cell.count('`') % 2 == 1:
            recovered.extend(_split_pipes(cell, respect_code=False))
        else:
            recovered.append(cell)
    cells = _strip_border_cells(recovered)
